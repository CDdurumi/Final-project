...
<title>Insert title here</title>
	...
	<script>
		$(document).ready(function(){
			// 0. 입력 정보를 활성화할 배열 선언
			const dataCheckArr = [false, false, false];
			...
			$("#sns-box").mouseover(function(){
				...
				if(true_stack == 3){
					...
					$("#data-submit").css("background", "#16a085");
				}
			});
			...
			<div id="sns-box" class="modal-content left" style="width:900px;">
					...
					<div class="modal-body" style="text-align:center; padding-bottom: 1rem; padding-left: 1rem; padding-right: 1rem;">
						<form action="/login/insertData" method="post">
							<input id="username" class="data-form" type="text" name="name" placeholder="이름" /> 
							<div class="notice_box"></div>
							
							<input id="email" type="text" class="data-form" name="email" placeholder="이메일" readonly/> 
							<div class="notice_box"></div> <!-- 기본 블루 -->
							
							<input id="nickname" type="text"  class="data-form" name="nickname" placeholder="닉네임" />
							<div class="notice_box"></div>
							
							<input id="phone" type="text" class="data-form" name="phone" placeholder="사용하시는 연락처 ('-'미포함)" />
							<div class="notice_box"></div>
